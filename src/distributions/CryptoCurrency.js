...
class CryptoSector {
    ...
    FetchData(Option) {
        ...
        var ReturnValue = { Raw: null, Refined: null };
        ...
        if (Option && Option.ID && Option.Type === 'null') {
            ...
            return NodeFetch(`${Url}`).then(response => response.json()).then(JsonResponse => {
                ReturnValue.Raw = JsonResponse[0];
                return ReturnValue;
            }).catch(error => { return error; });
        };
    };
